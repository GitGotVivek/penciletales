...
//                     <img src={logo} alt="Logo" className=" rounded-lg h-[85px] md:w-32px lg:w-[100px] bg-slate-100" />
//                 </a>
...
//             <div>
//                 <ul className={`md:hidden gap-12 text-lg block space-y-4 px-4 py-6 mt-24 bg-slate-900 ${isMenuOpen ? 'fixed top-0 left-0 w-full transition-all ease-out duration-200' : 'hidden'}`}>
//                     {navItems.map(({ path, link }) => (
...
const Navbar = () => {
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 w-full transition-all duration-300 z-50 ${isScrolled ? 'bg-black/70 backdrop-blur-md' : 'bg-black/25'}`}>
            <nav className='p-2 max-w-7xl mx-auto flex justify-between items-center'>
                <a href="/" className='text-md font-bold text-white flex items-center' title='Pencile Tales'>
                    <img src={logo} alt="Logo" className="h-20 sm:h-20 md:h-24 lg:h-28 xl:h-20 w-28 bg-gray-50 rounded-lg" />
                </a>
